...
---
[CzarniakMichelSedar Github Repository](https://github.com/ssedar/CzarniakMichelSedar) \
# Research Rationale
...
#Exporting to CSV:
write.csv(sums_intersection_tractlevel,
          'Data/Processed/EJI_Plants_TractLevel.csv', 
          row.names=FALSE)
...
This analysis initiates data exploration and visualization for the processed_data dataframe, focusing on fuel types, income, and electricity generation in North Carolina. Figure \@ref(fig:Figure1) generates a pie chart to illustrate the percentage makeup of primary fuel types in North Carolina based on total annual generation. Subsequently, the scatter plot in Figure \@ref(fig:Figure2) depicts the relationship between median household income and total plant annual generation, with points colored by fuel type. The exploratory analysis further drills down to scatter plots excluding nuclear data and for a selected set of fuel types (COAL, SOLAR, OIL, WIND, GAS). Additionally, stacked bar plots in Figures \@ref(fig:Figure5) and \@ref(fig:Figure6)  visualize the total annual generation by fuel type and county, and for better clarity, separate plots are created for the top 10 and bottom 10 counties based on median household income. These visualizations help explore and understand the relationships between fuel types, income levels, and electricity generation in different counties of North Carolina.

```{r Figure1, fig.cap="Pie chart illustrating the percentage distribution of primary fuel types in North Carolina based on total annual generation.", echo=FALSE, warning=FALSE, error=FALSE}
...
```{r Figure2, fig.cap="Scatter plot illustrating the correlation between median household income and total annual generation, color-coded by fuel type. Owing to the unique characteristics of nuclear power plants, operating at significantly higher capacity factors than other sources, the plot lacks a discernible relationship, prompting necessary adjustments.", echo=FALSE, warning=FALSE, error=FALSE}
...
```{r Figure3, fig.cap="Exploring energy patterns, this scatter plot analyzes the connection between median household income and total annual generation, excluding nuclear energy. The updated chart reveals a potential trend, especially around the $70,000 median income mark, where data points for generation are sparse.",echo=FALSE, warning=FALSE, error=FALSE}
#Scatter ex-nuclear
...
```{r Figure4,fig.cap="To narrow our exploration further, we selected key energy sources: coal, gas, oil, wind, and solar.", echo=FALSE, warning=FALSE, error=FALSE}
...
```{r Figure5, fig.cap="Stacked bar plot illustrating the total annual generation by fuel type in the top 10 counties (by median income) in North Carolina.",echo=FALSE, warning=FALSE, error=FALSE}
...
```{r Figure6, fig.cap="To get a balanced view of the total annual generation by fuel type and county, we then show the bottom 10 counties (by median income) in North Carolina. This identified Richmond county as a county of interest.",echo=FALSE, warning=FALSE, error=FALSE}
...
We then created initial maps to better understand the landscape of North Carolina in terms of socioeconomic status and geographic distribution of power plants. Figure \@ref(fig:Figure7) below shows the mapping of power plants across the state and helped us to visualize the concentration of power plants across the state. Figure \@ref(fig:Figure8) maps the nameplate capacity of those power plants by county. We noticed that multiple counties, especially Richmond county, stood out as having a high concentration of capacity. Figure \@ref(fig:Figure9) demonstrates a heat map of the percentage of each county's population living in poverty as of 2021. Richmond county is once again a clear visual outlier on this map.

```{r Figure7,fig.cap="Locations of active power plants across North Carolina as of 2021. Each black dot represents a power plant."}
...
```{r Figure8,fig.cap="This map shows the total nameplate capacity in Megawatts of each county in North Carolina. Multiple counties have no power plants and are blank. Richmond County is an interesting outlier which the highest capacity of all the counties."}
...
gen_cap_map <-ggplot(data=counties_sf_join)+
  ...
  theme(plot.margin=unit(c(0,0,-.40,0), "null"))
print(gen_cap_map)
```


```{r Figure9,fig.cap="Percent of total county population living in poverty in 2021, as defined by the USDA Economic Research Service."}
...
```{r Figure11,fig.cap="Total NOx Emissions in 2021 across North Carolina Power Plants."}
...
```{r Figure12,fig.cap="Total SO2 Emissions in 2021 across North Carolina Power Plants"}
...
```{r Figure13, fig.cap="Total CH4 Emissions in 2021 across North Carolina Power Plants."}
...
As exploratory work into the dimensions of social vulnerability, environmental burden, and health vulnerability in relation to generation and emissions across power plants in North Carolina, tests for normality were conducted on the overall index value for each of the aggregated themes (SVI, EBM, HVM). The distribution of the variables corresponding to these three overall themes of interest can be seen in Figures \@ref(fig:Figure14), \@ref(fig:Figure15), and \@ref(fig:Figure16).


```{r Figure14,fig.cap="Test for normality in distribution of Social Vulnerability Module percentile ranks. Normal Q-Q Plot does not show a totally normal distribution. Distribution is skewed such that most observations fall in the second half.", echo=FALSE}
...
```{r Figure15, fig.cap="Test for normality in distribution of Environmental Burden Module percentile ranks. Normal Q-Q Plot does not show a totally normal distribution. Distribution is skewed such that most observations fall in the second half.",echo=FALSE}
#Check normal distributions of continuous variables
par(mfrow = c(1,2), mar=c(6,4,6,4))
...
```{r Figure16, fig.cap="Test for normality in distribution of Environmental Burden Module percentile ranks. Distribution is shows that HVM (as Percentile Rank) is ordered but not continuous, the way EBM is. EJI obtains its data on indicators from a variety of sources. RPL_HVM, specifically, can take a value of 0, 0.2, 0.4, 0.6, 0.8, or 1.0.",echo=FALSE}
#Check normal distributions of continuous variables
par(mfrow = c(1,2), mar=c(6,4,6,4))
...
As the last set of geographic explorations, two maps were created. The first was a map of NC power plants overlaid on tract boundaries to ensure that the matching of tracts to power plants for based on spatial features for Q3 analyses was successful (see Figure \@ref(fig:Figure17)). The second maps asthma prevalence across NC tracts, as a visual grounding point in relation to Q3 analyses, given that the emissions considered in this project are known to have respiratory effects in humans (see Figure \@ref(fig:Figure18)).

```{r Figure17, fig.cap="Power Plants Across North Carolina Tracts", echo=FALSE}
#Map of power plants across NC tracts, shown over NC tract boundaries 
Plants_Tracts_Map <-mapview(tracts_containing_plants, 
            col.regions='snow', 
            layer.name = 'NC Tracts Containing Power Plants') +
   mapview(plants_within_tracts,
            layer.name = 'Plants Within Each Tract',
            col.regions='royalblue1',
            alpha=.5,
            cex=3)

Plants_Tracts_Map@map
```


```{r Figure18,fig.cap="Mapping Asthma Prevalence Across Tracts", echo=FALSE}
...
summary(linear_model_coal)$coef %>%
  kbl(caption = "Linear Regression Results - Median Income Impact on Coal Generation") %>%
...
```{r Figure19,fig.cap="Relationship of median household income ($) on coal generation (MWh) in 2021 by county.", warning=TRUE}
...
summary(linear_model_solar)$coef %>%
  kbl(caption = "Linear Regression Results - Median Income Impact on Solar Generation") %>%
  kable_classic(html_font = "Helvetica")
```
...
```{r Figure20,fig.cap="Relationship of median household income ($) on solar generation (MWh) in 2021 by county.", echo = FALSE}
#Visualize relationship
...
The next stage of analysis examined if there was a relationship between the quantity of generators and median household income. The linear regression results indicate that median household income does not show a statistically significant association with the number of plants. The coefficient is estimated to be -0.0000786, with a standard error of 0.0000922, a t-value of -0.85, and a p-value of 0.3962 (See Table \@ref(tab:Table3)). This suggests that, based on the available data, changes in median household income do not appear to be statistically linked to variations in the number of plants. Figure \@ref(fig:Figure22) below illustrates this relationship.

---

```{r Table3,echo=FALSE,warning=FALSE,error=FALSE}
...
summary(inc_plants_model)$coef %>%
  kbl(caption = "Linear Regression Results - Median Income Impact on Number of plants") %>%
  kable_classic(html_font = "Helvetica")
```

---

```{r Figure22,fig.cap="Relationship of Median Household Income and Number of Power Plants in NC in 2021 by county."}
#Plot relationship
...
summary(capacity_income_model)$coef %>%
  kbl(caption = "Linear Regression Results - Median Income Impact on Nameplate Capacity") %>%
  kable_classic(html_font = "Helvetica")
```

---

```{r Figure23,fig.cap="Relationship of Median Household Income on Average Nameplate Capacity (MW) by county in 2021."}
...
```{r Table9, echo=FALSE, warning=FALSE, error=FALSE}
...
A set of analyses was conducted to briefly explore whether there is a significant relationship between power plants and the social vulnerability index (SVI), more broadly. A linear regression of SVI themes on total annual generation by county generated results summarized in Table \@ref(tab:Table6). 
...
summary(Generation_County_lm)$coef %>%
  kbl(caption="Linear Regression Results - SVI and Total Generation") %>%
  kable_classic(html_font = "Helvetica")
...
```{r Table7,caption = "Linear Regression Results - Environmental Burden and Total Generation",echo=FALSE}
...
summary(EB_totalgen_lm)$coef %>%
  kbl(caption = "Linear Regression Results - Environmental Burden and Total Generation") %>%
  kable_classic(html_font = "Helvetica")
...
Another linear regression run on nameplate capacity (maximum possible generation) returned an R-squared of .00027 and a p-value of 0.77, meaning nameplate capacity could not be considered a worthwhile explanatory variable to explore for the overall environmental burden module either. Ultimately, these analyses were insufficient to reject the null hypothesis that there is no relationship between environmental burden and nameplate capacity, or power generation by tract. An existing relationship cannot be concluded. The lack of clear relationship is demonstrated in Figure \@ref(fig:Figure27) below.


```{r Figure27,fig.cap="The relationship between the overall Environmental Burden Module (EBM) percentile rank and total power generation by tract."}
#Visualize the linear regression
...
Bartlett's test was run to check the null hypothesis that the variance at each of the percentile ranks is the same, and yielded a K-squared of 237.49 with 5 degrees of freedom and a p-value < 2.2e-16. ANOVA (function 'aov') was then run on tract generation and RPL_HVM (see Figure \@ref(fig:Figure29) below), yielding a p-value well above 0.05 (0.385). The null hypothesis that the mean of generation is the same across percentile ranks therefore could not be rejected. Lastly, a Tukey HSD test was conducted to examine the differences between pairs of percentile ranks, but every p-value generated was above 0.05. 

```{r Figure29,fig.cap="Analysis of Variance performed on Generation and Health Vulnerability by Tract.", echo=FALSE}
...
summary(HV_totalgen_lm)$coef %>%
  kbl(caption = "Linear Regression Results - Health Vulnerability and Total Generation") %>%
  kable_classic(html_font = "Helvetica")

...
SVI_tract <-read.csv(here('Data/Raw/SVI_NorthCarolina_tract.csv'),
                  stringsAsFactors = TRUE)
...
#The estimated coefficient is approximately -134.6. This suggests that for each one-unit increase in income, the predicted value of coal generation decreases by approximately 134.6 MWh. However, the coefficient's p-value (0.1049) is greater than 0.05, indicating that it is not statistically significant at the 0.05 significance level. Additionally, the R^2 suggests that only about 13.24% of the variability in coal generation is explained by income.
...
#Merge SVI and average generation by county data


Generation_SVI <-merge(x = nameplate_capacity,
                           y = SVI_county, 
                           by.x = 'CNTYNAME', 
                           by.y = 'COUNTY' )

#Run multiple regression to determine if there is a relationship
Generation_County_lm <- lm(Sum_NAMEPCAP ~ RPL_THEMES + RPL_THEME1 + RPL_THEME2 + RPL_THEME3 + RPL_THEME4, data=Generation_SVI)
...
#Investigating if there is a relationship at the tract level

#Load in tract spatial data
census_tracts_sf<- st_read(here('Data/Spatial/tl_2021_37_tract.shp'))

mapview(census_tracts_sf)+mapview(plant_gen_sf)

#Draw buffer around power plant locations. Buffer is intended to be ~3 miles and assumes that 0.621371 mi=1km and 1km=1000 buffer units.
plant_buffer <- plant_gen_sf %>% st_buffer(4828.03)

#View the result
mapview(plant_buffer)

#Merge census tract shape files with SVI census tract data
SVI_tract[c('Location_Tract', 'Location_County','Location_State')] <- str_split_fixed(SVI_tract$LOCATION, ',', 3) #Reformat tract codes to match across data frames

SVI_tract_df <-merge(x = SVI_tract,
                           y = census_tracts_sf, 
                           by.x = 'Location_Tract', 
                           by.y = 'NAMELSAD')

SVI_tract_sf <-SVI_tract_df%>% 
  st_as_sf(coords = c('INTPTLON',
                      'INTPTLAT'),
           crs=4269)

#Merge plant and SVI data by tract using a geographic join
tract_plant <-st_join(plant_gen_sf, SVI_tract_sf)

#Analyze the data

tract_plant_df <- as.data.frame(tract_plant)
  
tract_generation <-tract_plant_df %>% group_by(NAME)%>% 
  summarize(Total_Generation = sum(PLNGENAN))
  
tract_generation_SVI <- merge(x=tract_generation,
                              y=SVI_tract_df,
                              by='NAME')

#Run multiple regression to determine if there is a relationship between power plant generation and SVI
Generation_Tract_lm_overall <- lm(Total_Generation ~ RPL_THEMES + NAME, data=tract_generation_SVI)

summary(Generation_Tract_lm_overall) 

#Graph the regression
tract_generation_SVI_plot <- ggplot(tract_generation_SVI, 

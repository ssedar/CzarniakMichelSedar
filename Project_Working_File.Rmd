...
There are 100 counties in North Carolina. There are 843 power plants made up of 1190 generators.
...
*eGRID - 2020 and 2021 data from the U.S. EPA on all power plants in the U.S. Data includes information on plant locations, production, retirements,and emissions.The 2020 data set was only utilized for historical retirement data as that was removed from the 2021 data set.

*Income - 2021 data from the USDA on percentage of people in each US county living in poverty. 

*Unemployment - 2000-2022 data from the USDA on unemployment rates in each US county.

*Social Vulnerability Index - 2020 data from the US CDC on key vulnerability criteria including socioeconomic status, household characteristics, racial & ethnic minority status, and house type & transportation.
...
#CONFIRM WHETHER TABLE IS NEEDED - IT'S IN THE RUBRIC SO I THINK YES?
...
*We then began data exploration and visualization for the processed_data dataframe, focusing on fuel types, income, and electricity generation in North Carolina. The summary(processed_data) provides a general overview of the dataset. Figure 1 illustrates the percentage makeup of primary fuel types in North Carolina based on total annual generation - the majority of which is natural gas. Subsequently, the scatter plot in figures 2 depicts the relationship between median household income and total plant annual generation, with points colored by fuel type. The analysis further drills down to scatter plots in figures 3 and 4 excluding nuclear data and for a selected set of fuel types (COAL, SOLAR, OIL, WIND, GAS). Additionally, stacked bar plots in figures 5, 6, and 7 visualize the total annual generation by fuel type and county, and for better clarity, separate plots are created for the top 10 and bottom 10 counties based on median household income. These visualizations help explore and understand the relationships between fuel types, income levels, and electricity generation in different counties of North Carolina.
...
pie_chart_percentage <- ggplot(fuel_summarized, aes(x = factor(1), fill = PLFUELCT, y = percentage)) + 
  labs(title = "North Carolina Primary Fuel (% of Total Annual Generation)",
       ...
       x = NULL, 
       caption = str_wrap("Figure 1: Pie chart illustrating the percentage distribution of primary fuel types in North Carolina based on total annual generation.",70)) + 
...
scatter_income_generation <- ggplot(processed_data, aes(x = MEDHHINC_2021, y = PLNGENAN, color = PLFUELCT)) +
  ...
  labs(title = "Relationship Between Annual Generation, Fuel Type, and Income",
       ...
       color = "Fuel Type",
       caption = str_wrap("Figure 2: Scatter plot depicting the relationship between median household income and total annual generation, with points colored by fuel type. Due to the nature of nuclear power plants generating far more energy, the plot fails to display any clear relationship, leading to necessary adjustments.",70))+
  theme(plot.caption = element_text(hjust = 0))
...
scatter_income_generation_ex_nuc <- ggplot(subset(processed_data, PLFUELCT != "NUCLEAR"), 
  ...
  labs(title = "Relationship Between Annual Generation, Fuel Type, and Income",
       ...
       y = "Total Plant Annual Generation (MWh)",
       color = "Fuel Type",
caption = str_wrap("Figure 3: Considering nuclear's idiosyncratic properties, the scatter plot depicting the relationship between median household income and total annual generation was regenerated and reflects a potential trend, particularly at the $70,000 median income mark.",70))+
  theme(plot.caption = element_text(hjust = 0))
...
scatter_income_generation_selected <- ggplot(selected_fuel_data, 
  ...
  labs(title = "Relationship Between Annual Generation, Fuel Type, and Income",
       ...
       y = "Total Plant Annual Generation (MWh)",
       color = "Fuel Type",
caption = str_wrap("Figure 4: To narrow our exploration further, we selected key energy sources: coal, gas, oil, wind, and solar.",70))+
  theme(plot.caption = element_text(hjust = 0))
...
stacked_barplot_total_generation <- ggplot(processed_data, aes(x = CNTYNAME, y = PLNGENAN, fill = PLFUELCT)) +
  ...
  labs(title = "Total Annual Generation by Fuel Type and County",
       ...
       y = "Total Plant Annual Generation (MWh)",
       fill = "Fuel Type",
caption = str_wrap("Figure 5: Stacked bar plot illustrating the total annual generation by fuel type and county in North Carolina. Given the number of counties in NC, this visualization did not prove fruitful in depicting any further analysis.",70)) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        plot.caption = element_text(hjust = 0))
print(stacked_barplot_total_generation) 

#Visualization of all counties is very messy --- split by top 10/bottom 10 income
...
stacked_barplot_top_income_counties <- ggplot(filtered_data_income, aes(x = CNTYNAME, y = PLNGENAN, fill = PLFUELCT)) +
  ...
  labs(title = "Total Annual Generation by Fuel Type and County (Top 10 by Median Income)",
       ...
       y = "Total Plant Annual Generation (MWh)",
       fill = "Fuel Type",
caption = str_wrap("Figure 6: To explore the total annual generation by fuel type and county in a more digestable manner, we first show the top 10 counties (by median income) in North Carolina.",70)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        plot.caption = element_text(hjust = 0))
...
stacked_barplot_bottom_income_counties <- ggplot(filtered_data_bottom_income, aes(x = CNTYNAME, y = PLNGENAN, fill = PLFUELCT)) +
  ...
  labs(title = "Total Annual Generation by Fuel Type and County (Bottom 10 by Median Income)",
       ...
       y = "Total Plant Annual Generation (MWh)",
       fill = "Fuel Type",
caption = str_wrap("Figure 7: To get a balanced view of the total annual generation by fuel type and county in a more digestable manner, we then show the bottom 10 counties (by median income) in North Carolina.",70)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        plot.caption = element_text(hjust = 0))
...
gens_counties_map <-ggplot()+
  ...
  labs(title = "Locations of NC Power Plants",caption=str_wrap("Figure 8:Black dots on this map represent the locations of power plants across North Carolina.",70))+
  theme(plot.caption = element_text(hjust = 0))
...
gen_cap_map <-ggplot(data=counties_sf_join)+
  geom_sf(aes(fill=NAMEPCAP_plant))+labs(title="Capacity by County",fill="Capacity (MW)",caption=str_wrap("Figure 9: This map shows the total nameplate capacity in Megawatts of each county in North Carolina. Multiple counties have no power plants and are blank. Richmond County is an interesting outlier which the highest capacity of all the counties.",70))+
  theme(plot.caption = element_text(hjust = 0))
gen_cap_map
...
income_heat_map <-ggplot(data=income_NC_sf_join)+
  geom_sf(aes(fill=PCTPOV017_2021))+labs(title= "% of County in Poverty - 2021", fill="%", caption=str_wrap("Figure 10: Percent of total county population living in poverty in 2021, as defined by the USDA Economic Research Service",70))+
  theme(plot.caption = element_text(hjust = 0))
...
CO2_map <-ggplot(data=CO2_counties_sf_join)+geom_sf(data=counties_sf)+
  ...
  labs(title="2021 CO2 Emissions",
       ...
       caption=str_wrap("Figure 11: Total CO2 Emissions in 2021 across North Carolina Power Plants",70))+
  theme(plot.caption = element_text(hjust = 0))
print(CO2_map)
...
NOx_map <-ggplot(data= NOx_counties_sf_join)+
  ...
  labs(title="2021 NOx Emissions",
       ...
       caption=str_wrap("Figure 12: Total NOx Emissions in 2021 across North Carolina Power Plants",70))+
  theme(plot.caption = element_text(hjust = 0))
print(NOx_map)
...
SO2_map <-ggplot(data=SO2_counties_sf_join)+
  ...
  labs(title="2021 SO2 Emissions",
       ...
       caption=str_wrap("Figure 13: Total SO2 Emissions in 2021 across North Carolina Power Plants",70))+
  theme(plot.caption = element_text(hjust = 0))
print(SO2_map)
...
N2O_map <-ggplot(data= N2O_counties_sf_join)+
  ...
  labs(title="2021 N2O Emissions",
       ...
       caption=str_wrap("Figure 14: Total N20 Emissions in 2021 across North Carolina Power Plants",70))+
  theme(plot.caption = element_text(hjust = 0))
print(N2O_map)
...
CH4_map <-ggplot(data= CH4_counties_sf_join)+
  ...
  labs(title="2021 CH4 Emissions",
       ...
       caption=str_wrap("Figure 15: Total CH4 Emissions in 2021 across North Carolina Power Plants",70))+
  theme(plot.caption = element_text(hjust = 0))
print(CH4_map)
...
scatter_coal <- ggplot(coal_data, aes(x = MEDHHINC_2021, y = PLNGENAN)) +
  ...
  labs(title = "Linear Regression for Coal Generation",
       x = "Median Household Income ($)",
...
## Question 1: What is the relationship between power plant size and socioeconomic status?

For the first research question, we theorized that counties with lower income levels will have larger power plants. We began first by analyzing income (based on poverty level data) against the number of power plants. A regression of these two variables produced a p-value of 0.3962 and is plotted in Figure [PLACEHOLDER] below. [PLACEHOLDER TO DISCUSS SECOND CHART...]
...
ggplot(county_plants_df, aes(x = MEDHHINC_2021, y = Num_Plants)) +
  ...
  labs(title = "Regression Analysis Between Power Plants and County Income",
       ...
       caption = str_wrap("Figure X: There is not a statistically significant relationship between the number of power plants and the county income level",70))+
  theme(plot.caption = element_text(hjust = 0))
...
ggplot(merged_data_capacity, aes(x = MEDHHINC_2021, y = NAMEPCAP_plant)) +
  ...
  labs(title = "Regression Analysis Between Nameplate Capacity and County Income",
       ...
       caption=str_wrap("Figure X:[ PLACEHOLDER]",70)) +
  theme(plot.caption = element_text(hjust = 0))
...
## Question 2: Is there a relationship between power plant retirements and unemployment?

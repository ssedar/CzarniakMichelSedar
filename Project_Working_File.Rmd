...
Data used in this analysis comes from a variety of sources. A brief overview of each source is outlined below and full citations can be found in the ReadMe file. More information on the metadata for each source can be found in the 'Metadata' folder.
...
#CONFIRM WHETHER TABLE IS NEEDED
...
pie_chart_percentage <- ggplot(fuel_summarized, aes(x = factor(1), fill = PLFUELCT, y = percentage)) + 
  labs(title = "North Carolina Primary Fuel (% of Total Annual Generation)",
       ...
       y = "Percentage of Total Generation (%)", x = NULL, caption = "Figure 1: Pie chart illustrating the percentage distribution of primary fuel types in North Carolina based on total annual generation.") +
...
scatter_income_generation <- ggplot(processed_data, aes(x = MEDHHINC_2021, y = PLNGENAN, color = PLFUELCT)) +
  ...
  labs(title = "Relationship Between Annual Generation, Fuel Type, and Income",
       ...
       color = "Fuel Type",
       caption = "Figure 2: Scatter plot depicting the relationship between median household income and total annual generation, with points colored by fuel type. Due to the nature of nuclear power plants generating far more energy, the plot fails to display any clear relationship, leading to necessary adjustments.")
...
scatter_income_generation_ex_nuc <- ggplot(subset(processed_data, PLFUELCT != "NUCLEAR"), 
  ...
  labs(title = "Relationship Between Annual Generation, Fuel Type, and Income",
       ...
       y = "Total Plant Annual Generation (MWh)",
       color = "Fuel Type",
caption = "Figure 3: Considering nuclear's idiosyncratic properties, the scatter plot depicting the relationship between median household income and total annual generation was regenerated and reflects a potential trend, particularly at the $70,000 median income mark.")
...
scatter_income_generation_selected <- ggplot(selected_fuel_data, 
  ...
  labs(title = "Relationship Between Annual Generation, Fuel Type, and Income",
       ...
       y = "Total Plant Annual Generation (MWh)",
       color = "Fuel Type",
caption = "Figure 4: To narrow our exploration further, we selected key energy sources: coal, gas, oil, wind, and solar.")
...
stacked_barplot_total_generation <- ggplot(processed_data, aes(x = CNTYNAME, y = PLNGENAN, fill = PLFUELCT)) +
  ...
  labs(title = "Total Annual Generation by Fuel Type and County",
       ...
       y = "Total Plant Annual Generation (MWh)",
       fill = "Fuel Type",
caption = "Figure 5: Stacked bar plot illustrating the total annual generation by fuel type and county in North Carolina. Given the number of counties in NC, this visualization did not prove fruitful in depicting any further analysis.") +
  theme_minimal() +
...
stacked_barplot_top_income_counties <- ggplot(filtered_data_income, aes(x = CNTYNAME, y = PLNGENAN, fill = PLFUELCT)) +
  ...
  labs(title = "Total Annual Generation by Fuel Type and County (Top 10 by Median Income)",
       ...
       y = "Total Plant Annual Generation (MWh)",
       fill = "Fuel Type",
caption = "Figure 6: To explore the total annual generation by fuel type and county in a more digestable manner, we first show the top 10 counties (by median income) in North Carolina.") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
stacked_barplot_bottom_income_counties <- ggplot(filtered_data_bottom_income, aes(x = CNTYNAME, y = PLNGENAN, fill = PLFUELCT)) +
  ...
  labs(title = "Total Annual Generation by Fuel Type and County (Bottom 10 by Median Income)",
       ...
       y = "Total Plant Annual Generation (MWh)",
       fill = "Fuel Type",
caption = "Figure 7: To get a balanced view of the total annual generation by fuel type and county in a more digestable manner, we then show the bottom 10 counties (by median income) in North Carolina.") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
gens_counties_map <-ggplot()+
  ...
  labs(title = "Locations of NC Power Plants",caption="Figure 8:Black dots on this map represent the locations of power plants across North Carolina.")
...
gen_cap_map <-ggplot(data=counties_sf_join)+
  geom_sf(aes(fill=NAMEPCAP_plant))+labs(title="Capacity by County",fill="Capacity (MW)",caption="Figure 9: This map shows the total nameplate capacity in Megawatts of each county in North Carolina. Multiple counties have no power plants and are blank. Richmond County is an interesting outlier which the highest capacity of all the counties.")
gen_cap_map
...
income_heat_map <-ggplot(data=income_NC_sf_join)+
  geom_sf(aes(fill=PCTPOV017_2021))+labs(title= "% of County in Poverty - 2021", fill="%", caption="Figure 10: Percent of total county population living in poverty in 2021, as defined by the USDA Economic Research Service")
...
CO2_map <-ggplot(data=CO2_counties_sf_join)+geom_sf(data=counties_sf)+
  ...
  labs(title="2021 CO2 Emissions",
       fill="CO2 Emissions (tn)",caption="Figure 11: Total CO2 Emissions in 2021 across North Carolina Power Plants")
print(CO2_map)
...
NOx_map <-ggplot(data= NOx_counties_sf_join)+
  ...
  labs(title="2021 NOx Emissions",
       fill=" NOx Emissions (tn)",caption="Figure 12: Total NOx Emissions in 2021 across North Carolina Power Plants")
print(NOx_map)
...
SO2_map <-ggplot(data=SO2_counties_sf_join)+
  ...
  labs(title="2021 SO2 Emissions",
       fill="SO2 Emissions (tn)",caption="Figure 13: Total SO2 Emissions in 2021 across North Carolina Power Plants")
print(SO2_map)
...
N2O_map <-ggplot(data= N2O_counties_sf_join)+
  ...
  labs(title="2021 N2O Emissions",
       fill=" N2O Emissions (lbs)",caption="Figure 14: Total N20 Emissions in 2021 across North Carolina Power Plants")
print(N2O_map)
...
CH4_map <-ggplot(data= CH4_counties_sf_join)+
  ...
  labs(title="2021 CH4 Emissions",
       fill="CH4 Emissions (lbs)",caption="Figure 15: Total CH4 Emissions in 2021 across North Carolina Power Plants")
print(CH4_map)
...
#Question 1: Are there more/larger power plants in counties with lower average socioeconomic status?
*[Description here]*

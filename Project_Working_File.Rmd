...
#Assign tracts to each plant  - FIX MEEEEEEEEEE
tract_plant <- plant_gen_sf %>% mutate(Tract=sf::st_intersects(plant_gen_sf, census_tracts_sf))

Generation_tract <-tract_plant %>%
  group_by(Tract)%>%
  summarize(Total_gen_tract = sum(PLNGENAN))

#Join the tract generation attributes to the SVI data
SVI_plants_join <-  merge(x = SVI_tract,
                           y = Generation_tract, 
                           by.x = 'FIPS', #OR LOCATION??????
                           by.y = 'Tract' )
```

...
#Transposing so FIPS code are columns across and years are reflected as years
...
summary(generator_retirement_data)

# Use table to count the occurrences of each unique FIPS code
fips_counts <- table(generator_retirement_data$FIPS_Code)

# Print counts for each FIPS code
for (fips in names(fips_counts)) {
  count <- fips_counts[fips]
...
ggplot() +
  ...
  labs(title = "Unemployment in Wayne and Rowan Counties",
       ...
       y = "Unemployment Rate (%)",
       color = "Dataset") +
  scale_color_manual(values = c("Rowan" = "blue", "Wayne" = "yellow")) +
  theme(legend.title = element_blank())  

# Line plot for Rowan and Wayne with legend
combined_plot <- ggplot() +
  geom_line(data = rowan_ue, aes(x = Year, y = `37159`, color = "Rowan"), size = 1) +
  geom_line(data = wayne_ue, aes(x = Year, y = `37191`, color = "Wayne"), size = 1) + 
  geom_vline(xintercept = 2012, linetype = "dashed", color = "black", size = 1) + #DASHED LINE NOT WORKING
  labs(title = "Unemployment in Wayne and Rowan Counties",
       x = "Year",
       y = "Unemployment Rate (%)",
       color = "Dataset") +
  scale_color_manual(values = c("Rowan" = "blue", "Wayne" = "green")) +
  theme(legend.title = element_blank()) 
...
#NEXT STEPS: Compare against county with no (or 1?) retirements
```

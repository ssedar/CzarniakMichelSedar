...
pie_chart_percentage <- ggplot(fuel_summarized, aes(x = "", fill = PLFUELCT, y = percentage)) +
  ...
  labs(
    ...
    x = NULL,
    caption = "Figure 1: Pie chart illustrating the percentage distribution of primary fuel \n types in North Carolina based on total annual generation.")
...
scatter_income_generation <- ggplot(processed_data, aes(x = MEDHHINC_2021, y = PLNGENAN, color = PLFUELCT)) +
  ...
  labs(title = "Relationship Between Annual Generation, Fuel Type, and Income",
       ...
       color = "Fuel Type",
caption = "Figure 2: Scatter plot depicting the relationship between median household income \n and total annual generation, with points colored by fuel type. Due to the nature of \n  nuclear power plants generating far more energy, the plot fails to display any \n clear relationship, leading to necessary adjustments.")
...
scatter_income_generation_ex_nuc <- ggplot(subset(processed_data, PLFUELCT != "NUCLEAR"), 
  ...
  labs(title = "Relationship Between Annual Generation, Fuel Type, and Income",
       ...
       y = "Total Plant Annual Generation (MWh)",
       color = "Fuel Type",
caption = "Figure 3: Considering nuclear's idiosyncratic properties, the scatter plot \n depicting the relationship between median household income and total annual generation was \n regenerated and reflects a potential trend, particularly at the $70,000 median income mark.")
...
scatter_income_generation_selected <- ggplot(selected_fuel_data, 
  ...
  labs(title = "Relationship Between Annual Generation, Fuel Type, and Income",
       ...
       y = "Total Plant Annual Generation (MWh)",
       color = "Fuel Type",
caption = "Figure 4: To narrow our exploration further, we selected key energy sources: \n coal, gas, oil, wind, and solar.")
...
stacked_barplot_top_income_counties <- ggplot(filtered_data_income, aes(x = CNTYNAME, y = PLNGENAN, fill = PLFUELCT)) +
  ...
  labs(title = "Total Annual Generation by Fuel Type and County (Top 10 by Median Income)",
       x = "County",
       y = "Total Plant Annual Generation (MWh)",
       caption = "Figure 5: Stacked bar plot illustrating the total annual generation by fuel type in the \n top 10 counties in North Carolina by income.",
       fill = "Fuel Type") +
...
stacked_barplot_bottom_income_counties <- ggplot(filtered_data_bottom_income, aes(x = CNTYNAME, y = PLNGENAN, fill = PLFUELCT)) +
  ...
  labs(title = "Total Annual Generation by Fuel Type and County (Bottom 10 by Median Income)",
       x = "County",
       y = "Total Plant Annual Generation (MWh)",
       caption = "Figure 6: To get a balanced view of the total annual generation by fuel type and county, \n we then show the bottom 10 counties (by median income) in North Carolina. This identified Richmond county as a county of interest.",
       fill = "Fuel Type") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
```{r, coal_viz, echo=FALSE, warning=FALSE, error=FALSE}
...
scatter_coal <- ggplot(coal_data, aes(x = MEDHHINC_2021, y = PLNGENAN)) +
  ...
  labs(title = "Linear Regression for Coal Generation",
       ...
       caption = "A visualization of the trend.")
print(scatter_coal)
...
```{r, num_plants_npcap, echo=FALSE, warning=FALSE, error=FALSE}
#Visualize relationship
...
```{r, NP_generation, echo=FALSE, warning=FALSE, error=FALSE}
...
knitr::kable(summary(generation_income_model)$coef, format = "html", caption = "Linear Regression Results - Generation")

```

## The linear regression suggests that there might be a relationship between income and generation. The statistically significant coefficient indicates that there is evidence to suggest that an increase in median household income is associated with a decrease in the predicted value of solar generation However, it's important to note that the explained variance is relatively low (about 0.56%), and other factors not included in the model may contribute to the variability in generation for solar.

```{r, viz_gen, echo=FALSE, warning=FALSE, error=FALSE}
...
print(combined_plot)
```
Here, counties with both a high and low number of generator retirements are identified to investigate whether fluctuations in unemployment rates are influenced by generator retirements or if the rates exhibit a similar trend regardless of such retirements. By selecting counties like Rowan and Wayne, which have the highest number of generator retirements, alongside counties like Burke and Craven with the least retirements, the exploratory analysis aims to discern any distinctive patterns in unemployment dynamics. As the plot shows instances in which the unemployment rates do not follow the hypothesized pattern, another variable was chosen for statistical analysis.

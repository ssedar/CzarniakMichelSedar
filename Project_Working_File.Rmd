...
*Environmental Justice Index - 2022 report including "indicators representing environmental burden, social vulnerability, and health vulnerability were selected based on a thorough literature review conducted by the EJI research team between December 2020 and December 2021." 
...
#KEEPEstablish shared GEOID format
EBM_countytract$CountyFIPS <- stringr::str_extract(EBM_countytract$GEOID, "^.{5}")
...
#KEEP map NOx emissions by county and overlay dots with color by tract-level asthma
#KEEP get 2021 spatial census data to convert tract GEOID to mapped coordinates
spatial_tracts <- tigris::tracts(state = 37)
class(spatial_tracts)
#KEEP mapview(spatial_tracts)

#KEEPJoin the eGrid attributes to the county spatial features
tracts_EJI_sf_join <-  merge(x = spatial_tracts,
                           y = EBM_countytract, 
                           by.x = 'GEOID', 
                           by.y = 'GEOID' )

#KEEPSelect variables of interest
tracts_EJI_sf_select <- tracts_EJI_sf_join %>%
  select(GEOID, CountyFIPS, TRACTCE.x, StateAbbr, COUNTY, E_TOTPOP, M_TOTPOP, 
         E_PM, E_DSLPM, E_COAL, E_LEAD, EP_UNINSUR, EP_AGE65, 
         EP_ASTHMA, EP_BPHIGH, EP_CANCER, EP_MHLTH, geometry) %>%
  #na.omit() %>% #selecting only data with retirements
  rename(TractCode = TRACTCE.x)  # Renaming the column

#KEEPMap asthma prevalence by tract
tracts_map <- ggplot(data = tracts_EJI_sf_select) +
...
#Merge EJI and total NOx emitted by county
NOx_EJI <-merge(x = Total_Generation_County,
                           y = EBM_select, 
                           by.x = 'CNTYNAME', 
                           by.y = 'COUNTY' )
...
#Exporting to CSV:
#write.csv(EBM_select,'Data/Processed/EBM_Processed.csv', row.names=FALSE)
...
# to what extent does proximity to coal explain PM -- COAL IS ZEROES, MOVE TO EXPLO?
coal_PM_lm<- lm(data = EBM_countytract, E_PM ~ E_COAL)
summary(coal_PM_lm)

# to what extent does proximity to coal explain asthma -- COAL IS ZEROES, MOVE TO EXPLO?
coal_asthma_lm<- lm(data = EBM_countytract, EP_ASTHMA ~ E_COAL)
summary(coal_asthma_lm)

# to what extent does proximity to coal explain high blood pressure --COAL IS ZEROES, MOVE TO EXPLO?

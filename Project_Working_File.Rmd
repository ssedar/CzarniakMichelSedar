...
-------------| -------------
Key Variables   | Cell 2
Data Hierarchy | Generators are associated to plants, plants are associated to counties and FIPS codes
Data Range | 2020 (data set used only to reference historical retirement data) and 2021 (all other data)
Data Source | U.S. EPA
...
*Income - 2021 data from the USDA on percentage of people in each US county living in poverty and median household income.
...
Power plants are known to be a major stationary point source of air pollution. As such, in the United States they are subject to regulation under the Clean Air Act, which sets National Ambient Air Quality Standards on six criteria air pollutants--particulate matter (PM2.5 and PM10), atmospheric ozone, carbon monoxide, lead, nitrogen dioxide, and sulfur dioxide. These pollutants reach widespread exposure (millions of people) due to their numerous and diverse sources. Causal and/or likely causal evidence pointing to negative health endpoints exists for each one of them. For carbon monoxide (CO), the regulatory standards have been informed, above all, by research on cardiovascular effects. For nitrogen dioxide (NO2) and sulfur dioxide (SO2), on the other hand, the standard-setting has been most heavily informed by research on respiratory effects. It should be noted that nitrogen oxides also contribute to the formation of atmospheric ozone.

While the Clean Air Act and its various amendments have been in effect for decades, its effectiveness in reducing air pollution in the U.S. by way of regulating emissions of criteria air pollutants has varied over time and varied by pollutant. Air pollution continues to affect millions in the United States. This study uses data from CDC's Environmental Burden piece of its Environmental Justice Index to test the hypothesis that greater presence of power plants in a county makes that county more likely to see elevated NOx and SOx levels, and consequently more respiratory and cardiovascular problems in comparison to those counties with fewer power plants.
...
EBM_countytract$CountyFP <- GEOID = stringr::str_extract(EBM_countytract$GEOID, "^.{5}")

#Add nameplate capacity by county to EBM data for analysis
EBM_select <- EBM_countytract %>% 
  select(COUNTY, CountyFIPS, EP_ASTHMA, EP_BPHIGH, EP_UNINSUR, 
         E_PM, E_OZONE, E_DSLPM) %>%
  rename(CNTYNAME = COUNTY)  # Renaming the column
EBM_nameplate_merge <- merge(EBM_select, nameplate_capacity, by = "CNTYNAME")

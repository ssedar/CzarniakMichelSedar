...
ts_data <- ts(processed_data_20$GENYRRET, start = min(processed_data_20$YEAR), frequency = 1)


# Seasonality decomposition
seasonal_decomposition <- decompose(ts_data)
seasonal_plot <- plot(seasonal_decomposition)

# Step 3: Trend Analysis using tseries::trend.test
trend_test_result <- trend.test(ts_data)
print(trend_test_result)

# Step 4: Addressing Seasonality
# If seasonality is detected, you may consider deseasonalizing the data
deseasonalized_data <- ts_data - seasonal_decomposition$season
deseasonalized_plot <- plot(deseasonalized_data, col = "green")

# Step 5: Covariance Analysis
# Compute the covariance matrix (assuming other columns are in the data frame)
cov_matrix <- cov(processed_data_20[, c("Unemployment_rate_2000", "Unemployment_rate_2001", "Unemployment_rate_2022", "GENYRRET")])

# Step 6: Modeling (linear model)
model <- lm(GENYRRET ~ Unemployment_rate_2000 + Unemployment_rate_2001 + ... + Unemployment_rate_2022, data = processed_data_20)
summary(model)

# Step 7: Heteroscedasticity Analysis

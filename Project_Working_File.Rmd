...
#Combining Gen20 and Plant df based on ORISPL
plant_gen20 <- gen20 %>%
  inner_join(plant_select, by = 'ORISPL', 
             suffix = c("_gen", "_plant"))
...
##### WRANGLING UE DATASET ########

colnames(ue)

transposed_ue <- ue %>%
  filter(str_starts(FIPS_Code, "37")) %>%  # Filter to include only FIPS_Codes starting with 37
...
print(generator_retirement_data)
print(transposed_ue)

# Convert "Year" column to numeric in transposed_ue
transposed_ue$Year <- as.numeric(transposed_ue$Year)

# Rename columns if needed
colnames(generator_retirement_data) <- c("FIPS_Code", "Year")

# Join the dataframes
merged_data <- inner_join(transposed_ue, generator_retirement_data, by = c("FIPS_Code", "Year"))


merged_data <- inner_join(transposed_ue, generator_retirement_data, by = c("FIPS_Code", "Year"))

# Check column names of transposed_ue
print(colnames(transposed_ue))

# Check column names of generator_retirement_data
print(colnames(generator_retirement_data))
...
######## TIME SERIES STUFF ################
processed_data_20$YEAR <- as.Date(processed_data_20$YEAR, format = "%Y")

ts_data <- ts(processed_data_20$GENYRRET, start = min(processed_data_20$YEAR), 
              frequency = 1)

# Extract relevant columns
unemployment_columns <- grep("^Unemployment_rate_", names(processed_data_20), value = TRUE)
generator_retirement_column <- "GENYRRET"

# Subset the data
subset_data <- processed_data_20[, c(unemployment_columns, generator_retirement_column)]

# Convert to ts (time series)
ts_data <- ts(subset_data[, generator_retirement_column], start = 1, frequency = 1)
...
# Regression analysis
model <- lm(subset_data[, generator_retirement_column] ~ ., data = subset_data)
summary(model)
...
# Subset the data
subset_data <- processed_data_20[, c("FIPS_Code", unemployment_columns, generator_retirement_column)]
